...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadPremiumArticles() {
    ...
    try {
      ...
      console.error('Erreur lors du chargement des articles premium:', error);
      container.innerHTML = '<p> Impossible de charger les articles premium. Veuillez actualiser la page.</p>';
    }
  }

  function renderArticles(articles, container) {
    if (!articles || articles.length === 0) {
      container.innerHTML = '<p>Aucun article premium disponible pour le moment.</p>';
      return;
    }
